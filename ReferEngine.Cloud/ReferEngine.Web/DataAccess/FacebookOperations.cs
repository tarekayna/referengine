...
using ReferEngine.Web.Utilities;
    ...
    public class FacebookOperations
    {
        ...
        public async Task<IList<Person>> GetFriendsAsync()
        {
            if (_friends == null)
            {
                ...
                // TODO: remove this when facebook tagging is authorized
                Person currentUser = await GetCurrentUserAsync();
                _friends.Add(currentUser);
            }
            return _friends;
        }
        ...
        public static async Task<FacebookAccessToken> ExchangeCodeForTokenAsync(string accessCode)
        {
            string query = String.Format("client_id={0}&redirect_uri={1}&client_secret={2}&code={3}",
                                         ReferEngineAppId,
                                         "https://www.referengine.com/refer/win8/success",
