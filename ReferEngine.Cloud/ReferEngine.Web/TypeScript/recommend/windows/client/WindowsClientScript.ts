...
export module ReferEngine {
        ...
        static authorizeFacebookAsync() {
            return new WinJS.Promise(function (comp, error, prog) {
                ...
                var query = "client_id=" + referEngineFacebookAppId +
                            ...
                            "&scope=email,publish_actions&display=popup&response_type=code";
    ...
    var functions: Functions.Function[] = [
        ...
        new Functions.Function(clientFunction.authApp, function (details) {
            Auth.authorizeAppAsync().done(function (authResult: Messaging.AuthResult) {
                messenger.send(new Messaging.Message("auth-app-result", { authResult: authResult }));
            });
        }),
