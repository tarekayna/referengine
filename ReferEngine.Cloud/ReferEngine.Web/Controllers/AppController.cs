...
namespace ReferEngine.Web.Controllers
{
    ...
    public class AppController : BaseController
    {
        ...
        public ActionResult GetAppDashboardMapData(long id, string who, string when)
        {
            ...
            {
                case "past-24-hours":
                    start = end.Subtract(TimeSpan.FromHours(24));
                    break;
                case "past-48-hours":
                    start = end.Subtract(TimeSpan.FromHours(48));
                    break;
                case "past-3-days":
                    start = end.Subtract(TimeSpan.FromDays(3));
                    break;
                case "past-7-days":
                    start = end.Subtract(TimeSpan.FromDays(7));
                    break;
                case "past-30-days":
                    start = end.Subtract(TimeSpan.FromDays(30));
                    break;
                case "past-60-days":
                    start = end.Subtract(TimeSpan.FromDays(60));
                    break;
                case "past-90-days":
                    start = end.Subtract(TimeSpan.FromDays(90));
                    break;
                case "past-year":
            ...
            {
                case "launched":
                    locations = DatabaseOperations.GetAppLaunchLocations(ViewProperties.CurrentApp, start, end);
                    break;
                case "intro":
                    locations = DatabaseOperations.GetAppIntroLocations(ViewProperties.CurrentApp, start, end);
                    break;
                case "recommended":
                    locations = DatabaseOperations.GetAppRecommendLocations(ViewProperties.CurrentApp, start, end);
                    break;
                default:
                    return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            }
